...
function search() {
    if ($.fn.dataTable.isDataTable('#TaggingTable')) {
        $('#TaggingTable').DataTable().destroy();
        ToDatatable();
    }
    else {
        ToDatatable();
...
function ToDatatable() {
   // var editval1 = 'yes';
    ...
    var url = '/Phases/Phase1List';

    var table = $('#TaggingTable').DataTable({
        ...
        "aoColumns": [
            {
                ...
                render: function (data, type, row, meta) {
                    return '<a href="/jobs/jobsinitial/' + row[1] + '?flag='+row[7]+'" data-id="' + row[1]+'" class="jobedit">' + row[3] + '</a>';
